...
| Tool | What it does |
|---|---|
| `duck_list_verbs` | Every verb, its params, safety class, and whether the current contract allows it. Call this first. |
| `duck_run_verb(name, params)` | Run any verb (`search_scan`, `walk_to`, `kick`, `grab`, `gaze`, …). Refusals come back as `ok: false`. |
...
```json
{
  "mcpServers": {
    "quackd": {
      "command": "uvx",
      "args": ["quackd", "serve-mcp", "--transport", "sim2d"]
    }
  }
}
```
...
```
quackd serve-mcp --transport sim2d --seed 7          # a different world
...
quackd serve-mcp --transport jsonrpc --address tcp://127.0.0.1:9870   # real robot, experimental
```

## The two-minute script

1. `claude mcp add quackd -- uvx quackd serve-mcp --transport sim2d` (≈20 s, first run downloads quackd)
